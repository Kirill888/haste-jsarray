...
newtype JSArray a = JSArray JSAny deriving (FromAny, ToAny)
...
head::FromAny a=>JSArray a->IO (Maybe a)
head arr = do
  len <- length arr
  case len of
    0 -> return Nothing
...
last::FromAny a=>JSArray a->IO (Maybe a)
last arr = do
  len <- length arr
  case len of
    0 -> return Nothing
...
fmap::(FromAny a, ToAny b, NFData a)=>(a->b)->JSArray a->JSArray b
...
foldl::(FromAny a, FromAny b, ToAny b)=>(b->a->b)->b->JSArray a->b
foldl f x0 arr = unsafePerformIO $ foldl_ (\x y -> return $ f x y) x0 arr
  where
    foldl_::(FromAny a, FromAny b, ToAny b)=>(b->a->IO b)->b->JSArray a->IO b
...
concatMap::(FromAny a, NFData a)=>(a->JSArray b)->JSArray a->JSArray b
concatMap f = concat . fmap f

concatMap'::(FromAny a, ToAny b, NFData a)=>(a->[b])->JSArray a->JSArray b
concatMap' f = concatMap (fromList . f)

replicate::ToAny a=>Int->a->JSArray a
replicate n x = unsafePerformIO $ replicate' n x
  where
    replicate'::ToAny a=>Int->a->IO (JSArray a)
...
push::ToAny a=>JSArray a->a->IO ()
push = ffi "(function(arr, val) {arr.push(val);})"

pop::FromAny a=>JSArray a->IO a
pop = ffi "(function(arr) {return arr.pop();})"

(!)::FromAny a=>JSArray a->Int->IO a
(!) = ffi "(function(arr, idx) {return arr[idx];})"

zipWith::(FromAny a, FromAny b, ToAny c)=>(a->b->c)->JSArray a->JSArray b->JSArray c
zipWith f xs ys = unsafePerformIO $ zipWith' (\x y -> return $ f x y) xs ys
  where
    zipWith'::(FromAny a, FromAny b, ToAny c)=>(a->b->IO c)->JSArray a->JSArray b->IO (JSArray c)
...
fromList::ToAny a=>[a]->JSArray a
fromList vals = unsafePerformIO $ do
  arr <- newJSArray
  mapM_ (push arr) vals
  return arr

toList::(FromAny a, NFData a)=>JSArray a->[a]
